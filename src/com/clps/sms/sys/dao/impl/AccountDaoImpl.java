...
//	通过名字查询所有数据
	private static final String SELECTACCOUNTSBYNAME = "SELECT  "+strtemp+" FROM ACCOUNT WHERE ACC_NAME=?";
	...
	public boolean queryAccountByName(String acc_name) throws SQLException{
		...
		rs=ps.executeQuery();
		flag=showcheckres(rs);
		logger.info("queryAccountByName checkName:"+flag+"------"+listAccount.size());
		super.getClose();
		return !flag;
	}
...
//		密码加密
	...
	public boolean insertAccount(Account acc) throws SQLException {
		...
		ps.setString(1,acc.getAcc_name());
		ps.setString(2,acc.getAcc_password());
		ps.setString(3,acc.getAcc_email());
		ps.setString(4,acc.getAcc_mobile());
		ps.setString(5,acc.getAcc_dec());
		ps.setInt(6,acc.getAcc_status());
		ps.setString(7,acc.getAcc_created_date());
		ps.setInt(8,acc.getAcc_created_name());
		ps.setInt(9,acc.getAcc_role_id());
	...
	private List<Account> setListOfAccount(ResultSet rs) throws SQLException{
		...
		while(rs.next()){
			...
			account.setAcc_updated_date(rs.getString("acc_updated_date"));
			account.setAcc_updated_name(rs.getInt("acc_updated_name"));
	 ...
	 */
	@Override
	public Account getAccountByName(String acc_name) throws SQLException {
		// TODO Auto-generated method stub
		this.ps = super.getConnection().prepareStatement(SELECTACCOUNTSBYNAME);
		ps.setString(1, acc_name);
		this.rs = ps.executeQuery();
		setListOfAccount(rs);
		logger.info("listOfAccount size: "+listAccount.size());
