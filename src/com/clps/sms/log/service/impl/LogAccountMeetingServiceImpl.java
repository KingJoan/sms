...
public class LogAccountMeetingServiceImpl implements LogService{
	...
	public JSONArray listLogsByName(String action, String action_content, String type, String created_name, int page) {
		...
		int num=Log.PAGESIZE;
		int begin=page*Log.PAGESIZE;	
		List<AccountMeetingLog> accountMeetingLogs=accountMeetingLogDao.queryAccountMeetingLogs(action, action_content, type, created_name, begin, num);
		System.err.println("name-------------"+accountMeetingLogs.size());
		return parseListToJson(accountMeetingLogs,page);
	}
