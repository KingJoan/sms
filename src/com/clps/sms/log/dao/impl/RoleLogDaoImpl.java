...
public class RoleLogDaoImpl extends BaseDaoImpl<RoleLog> implements RoleLogDao {
	...
	private static final String HEAD="SELECT ROLE_LOG.RL_ID, ROLE_LOG.RL_ACTION, ROLE_LOG.RL_ACTION_CONTENT, ROLE_LOG.RL_CREATED_DATE, ROLE_LOG.RL_CREATED_ID, ROLE_LOG.RL_TYPE, ACCOUNT.ACC_NAME FROM ROLE_LOG , ACCOUNT WHERE ROLE_LOG.RL_CREATED_ID = ACCOUNT.ACC_ID ";
	...
	private static final String QUERYROLELOGSBYTIMERANGE=HEAD+"AND ROLE_LOG.RL_ACTION LIKE ? AND ROLE_LOG.RL_ACTION_CONTENT LIKE ? AND ROLE_LOG.RL_TYPE LIKE ? AND ROLE_LOG.RL_CREATED_DATE >= ? AND ROLE_LOG.RL_CREATED_DATE <= ? LIMIT ?,  ?";
	
	private static final String QUERYROLELOGSBYCREATEDNAME=HEAD+"AND ROLE_LOG.RL_ACTION LIKE ? AND ROLE_LOG.RL_ACTION_CONTENT LIKE ? AND ROLE_LOG.RL_TYPE LIKE ? AND ACCOUNT.ACC_NAME = ? LIMIT ?,  ?";
	
	private static final String QUERYROLELOGSBYCREATEDNAMEANDTIMERANGE=HEAD+"AND ROLE_LOG.RL_ACTION LIKE ? AND ROLE_LOG.RL_ACTION_CONTENT LIKE ? AND ROLE_LOG.RL_TYPE LIKE ? AND ACCOUNT.ACC_NAME = ? AND ROLE_LOG.RL_CREATED_DATE >= ? AND ROLE_LOG.RL_CREATED_DATE <= ? LIMIT ?,  ?";
